...
def retrieve_context_with_reranking(student_profile: str):
    ...
    # 1. 載入兩個不同層級的模型
    print("⏳ 正在載入模型...")
    embed_model = SentenceTransformer('./SentenceTransformer',local_files_only=True)
    rerank_model = CrossEncoder('./CrossEncoder',local_files_only=True)

    # 2. 連線資料庫
    conn = psycopg2.connect(dbname="vectordb", user="myuser", password="mypassword", host="localhost", port="5432")
